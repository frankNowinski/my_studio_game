require 'studio_game/game'

module StudioGame
	
	describe Game do
		before do
			$stdout = StringIO.new
		end

		before do
			@game = Game.new("Test")

			@health = 100
			@player = Player.new("Frank", @health)

			@game.add_player(@player)
		end

		it "has a title" do
			expect(@game.title).to eq "Test"
		end

		it "w00ts the player if the number rolled is greater than 4" do
			allow_any_instance_of(Die).to receive(:roll).and_return(5)

			@game.play(2)

			expect(@player.health).to eq @health + (15 * 2)
		end

		it "skips the player when a three is rolled" do 
			allow_any_instance_of(Die).to receive(:roll).and_return(3)

			@game.play(2)

			expect(@player.health).to eq @health
		end

		it "blams the players when less than a three is rolled" do
			allow_any_instance_of(Die).to receive(:roll).and_return(2)

			@game.play(2)

			expect(@player.health).to eq @health - (10 * 2)
		end

		it "assigns a treasure for points during a player's turn" do
			game = Game.new("Test")
			player = Player.new("frank")
			game.add_player(player)

			game.play(1)

			expect(player.points).not_to be 0
		end

		it "computes total points as the sum of all player points" do 
			game = Game.new("Test")

			player1 = Player.new("Frank")
			player2 = Player.new("Tina")
			game.add_player(player1)
			game.add_player(player2)

			player1.found_treasure(Treasure.new(:hammer, 50))
			player2.found_treasure(Treasure.new(:crowbar, 400))

			expect(game.total_points).to eq 450
		end	
	end
end















